...
/// The same, with `config` written to a file it is pointed at.
...
impl Bridge {
    /// The binary on `config`, once it has said where it listens.
    pub fn started(config: &str) -> Bridge {
        let mut child = command(config).spawn().expect("the binary starts");
